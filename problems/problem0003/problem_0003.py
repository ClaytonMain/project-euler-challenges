...
# Preface: I'm not going to be putting detailed explanations in all my
# solution scripts, just ones I found more intriguing than others, or ones
# for which I took multiple approaches, or when I just feel like it.
# Anywho, preface is over now.
#
# So, finding all the primes less than 600851475143 (henceforth known as
# "the prompt number/value/whatever"), then looping through those primes and
# attempting to divide the prompt number by said primes, then returning the
...
#       (so, 2, initially).
#   2.  We're going to call the original prompt number (n). Make a
#       copy of n (copy_n).
#   3.  Try to divide copy_n by p. If p divides copy_n (p|copy_n),
#       overwrite copy_n with copy_n/p, then go back to 2.
#   4.  After dividing copy_n by p as much as possible, set p to our
#       next known prime.
#   5.  If p is now >= copy_n, then we know there are no larger primes
#       to check, and copy_n should now be equal to the largest prime
#       that will divide n, so return copy_n. Otherwise go back to 2.
#
...
def p0003_solution02(n=600851475143):
    ...
    for p in range(2, maxp):
        ...
        # I'm pretty sure I could change this to p == copy_n, but I don't
        # feel like it, so I won't.
        if p >= copy_n:
            break
        # So, since p is just generated from range(), we don't actually know
        # if p is prime or not, so gotta check that. Maximillion Prime here
...
# value*. It's actually super inefficient for numbers with large prime
...
def p0003_solution03(n=600851475143):
    ...
    # "Hold your fucking horses", I reply.
    ...
    copy_n = n
    composites = set()
    # Also, we're bringing this guy back.
    maxp = math.ceil(math.sqrt(copy_n)) + 1
    ...
    for p in range(2, maxp):
        ...
        # If it's composite, we dont' care about it and we restart the loop.
        if p in composites:
            continue
        ...
        # them to the set of composites for future reference. This right here
        # is why solution 1 is faster when n has small prime factors.
        composites.update(range(p * p, maxp, p))
    # And again, once done looping we just return copy_n.
    return int(copy_n)


# So accessing sets is pretty cool. Yeah, I know. It's tight.
# Anywho, let's see some calculation speeds for values of n.
...
def p0003_solution04(n=600851475143):
    ...
    copy_n = n
    # But what is *this*?
    # Primes! Except right now it's not *strictly* primes. They're potential
    # primes. But yeah, for now it's all values from 2 to the sqrt of n.
    # I wanted to tackle solution 3 using a set of primes, but I figured
    # the extra function calls would be too much overhead, so I just used
    # composites in that solution. But more on that later.
    maxp = math.ceil(math.sqrt(copy_n)) + 1
    ...
    for p in range(2, maxp):
        ...
        maxp = math.ceil(math.sqrt(copy_n)) + 1
        # So anywho, then I foun out about the difference_update function,
        # so that was tight. An seemingly efficient way to handle this.
        # But...
...
# update function. So anywho, the solution I like most is definitely
# solution 3. Here it is below without all those pesky comments cluttering
# everything up.
